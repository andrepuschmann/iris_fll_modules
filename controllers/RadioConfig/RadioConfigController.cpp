...
namespace iris
{
    ...
    void RadioConfigController::initialize()
    {
        ...
        thread_.reset( new boost::thread( boost::bind( &RadioConfigController::workFunction, this ) ) );
    }
    ...
    void RadioConfigController::workFunction()
    {
        try {
            ...
            {
                LOG(LINFO) << "Work thread interrupted." << std::endl;
            }
        }
        catch(IrisException& ex)
        {
            LOG(LERROR) << "Error in RadioConfigController: " << ex.what() << std::endl;
        }
    ...
    void RadioConfigController::handleRequest()
    {
        ...
        // call recv function in a non-blocking fashion (needed to kill thread properly)
